...
\begin{figure}[t]
  ...
  \caption{What each planner pays for reliability, over the 50 randomized
    scenarios. Horizontally, the fraction solved; vertically, the median
    primary path length, normalized by the lone-agent reference length
    $L_\mathrm{ref}$ and plotted downward, so \emph{up and to the right is
    better}. Marker shape and dash pattern identify the planner; each line is
    the trajectory it sweeps out as the terminal uncertainty bound tightens,
    and the marker fills step through the shared key below, green (loose) to
    dark violet (tight). Ours holds the top-right corner: it is still solving 80\% of the
