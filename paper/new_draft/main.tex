...
\begin{figure}[t]
  ...
  \caption{Fraction of the 50 randomized scenarios each planner solved as the
    ...
    the color along each line encodes what that success cost --- the median
    primary path length at that level, normalized by the lone-agent reference
    length $L_\mathrm{ref}$, on the shared scale below the axis. One ramp
    serves every planner, so the same color means the same detour on every
    line.}
