...
\begin{figure}[t]
  \centering
  \figbox{5cm}{Fig.~3 --- x: constraint level, y: path-length ratio,
    one line per planner, line opacity = success rate at that level}
  \caption{Path length versus constraint level across 50 randomized scenarios.
    Opacity encodes the fraction of scenarios each planner solved at that
    level; a faint line is a planner reporting short paths on the few easy
    instances it could still solve. \todo{write real caption}}
  \label{fig:length_success}
\end{figure}

\begin{figure}[t]
  \centering
  \figbox{4cm}{Fig.~4 --- vertical box plot of wall-clock time per planner}
  \caption{Wall-clock time per planner over all successful runs.
    \todo{write real caption}}
  \label{fig:wall}
\end{figure}
