...
The planning problem gathers the objective and all constraints in one place:
\begin{subequations}
\label{eq:opt}
\begin{align}
...
deviation $\sigma$ for an isotropic covariance $\sigma^2 I$ and is therefore
expressed in metres.

\textbf{Obstacle avoidance \eqref{eq:cc}.} Every agent must clear every
obstacle with probability at least $1-\delta$ at every instant: the constraint
must hold for \emph{some} face $j$ of each obstacle $\mathcal{O}_o$, for every
agent $i$, obstacle $o$, and arc position $s$. The mean then lies outside one
face with a margin covering the vehicle radius $r_a$ and both the agent's and
the obstacle's positional uncertainty, where $z_\delta = \Phi^{-1}(1-\delta)$
and $\Phi$ is the standard normal distribution function
\todo{cite Blackmore / Ono--Williams}. A union bound over the $PM$
...
positions with a common prior covariance, and the primary must end at the goal.

The belief dynamics do not appear as constraints: $\Sigma_i(s)$ is a
deterministic functional of the decision variables through
...
\eqref{eq:ci}: the supports' trajectories affect the problem solely by shaping
the primary's covariance, which is what makes the problem a joint one.
...
\begin{figure}[t]
  ...
  \caption{Baseline comparison over the 50 randomized scenarios as the terminal
    ...
    $U_\mathrm{ref}$. (a)~Fraction of scenarios solved. (b)~Primary path length
    over the solved scenarios, normalized by the lone-agent reference length
    $L_\mathrm{ref}$: mean $\pm$ one standard deviation, planners offset
    slightly along $x$ for legibility. Sequential's bars at 40\% and 30\%
    ($2.5 \pm 1.4$ and $2.8 \pm 1.5$) run off the top of the axis. Ours still
    solves 80\% of the scenarios at half the reference bound, at a mean cost
    of $1.34\,L_\mathrm{ref}$.}
...
\begin{figure}[t]
  ...
  \caption{Ablation on a separate set of 30 randomized scenarios, drawn without
    ...
    (a)~success rate, (b)~mean $\pm$ SD path length over the solved scenarios.
...
\begin{table}[t]
  \caption{Success rate (\%) / mean $\pm$ SD of the primary path length over
    the solved scenarios, normalized by $L_\mathrm{ref}$, at four constraint
    levels. Top block: the 50 screened scenarios of
    ...
    on which the full pipeline is listed again; ``straight seed'' is the
    continuous stage seeded from a straight line. ``--'': no scenario solved.}
  \label{tab:sweep}
  \centering
  \scriptsize
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    Method & 100 & 70 & 50 & 30 \\
    \midrule
    Ours          & 100 / 1.14$\pm$0.09 & 100 / 1.22$\pm$0.23 & 80 / 1.34$\pm$0.26 & 40 / 1.41$\pm$0.21 \\
    Formation     & 98 / 1.22$\pm$0.12 & 72 / 1.34$\pm$0.25 & 54 / 1.33$\pm$0.16 & 22 / 1.48$\pm$0.45 \\
    Sequential    & 100 / 1.18$\pm$0.14 & 94 / 1.41$\pm$0.92 & 52 / 1.36$\pm$0.50 & 16 / 2.76$\pm$1.51 \\
    CL-GBT        & 100 / 1.47$\pm$0.33 & 70 / 1.78$\pm$0.43 & 32 / 2.01$\pm$0.41 & 6 / 2.07$\pm$0.26 \\
    Greedy        & 90 / 1.24$\pm$0.10 & 14 / 1.21$\pm$0.06 & 4 / 1.13$\pm$0.01 & 4 / 1.13$\pm$0.01 \\
    \midrule
    Full pipeline & 100 / 1.12$\pm$0.14 & 100 / 1.17$\pm$0.23 & 87 / 1.22$\pm$0.22 & 43 / 1.29$\pm$0.22 \\
    Discrete only & 100 / 1.20$\pm$0.15 & 100 / 1.23$\pm$0.23 & 80 / 1.25$\pm$0.22 & 43 / 1.29$\pm$0.22 \\
    Straight seed & 53 / 0.99$\pm$0.04 & 7 / 1.00$\pm$0.00 & -- & -- \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{Closed-loop Monte Carlo of the behind-wall plan in HoloOcean, 30
    runs with different noise seeds. Top: planned trajectories (dashed) against
    the mean executed trajectory of each agent, with the $\pm1\sigma$
    cross-track envelope drawn on the map; the primary passes north of the
    obstacle while the support diverts south to the landmark and rejoins before
    the goal. Bottom: the same envelope at true scale against arc length. It is
    a 2--3\,m tube along the 1.1\,km route and stays within about 4\,m; every
    run reaches the goal and clears the obstacle.}
  \label{fig:mc_paths}
\end{figure}
