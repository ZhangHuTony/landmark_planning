...
\begin{figure}[t]
  ...
  \caption{Fraction of the 50 randomized scenarios each planner solved as the
    terminal uncertainty bound tightens. Marker shape identifies the planner,
    keyed along the bottom; the color along each line encodes what that success
    cost --- the median primary path length at that level, normalized by the
    lone-agent reference length $L_\mathrm{ref}$, on the shared scale below the
    axis. One ramp serves every planner, so the same color means the same
    detour on every line. Ours (yellow) is still solving 80\% of the scenarios at
    half the reference bound, and pays under $1.31 L_\mathrm{ref}$ to do it.}
...
\begin{figure}[t]
  ...
  \caption{Wall-clock time per planner over successful runs, split by
    ...
    the interquartile range, whiskers extend to $1.5\times$ IQR, and dots are
    individual runs beyond them. Four of the five planners cost the same
    whatever the bound; only CL-GBT's search grows with it, its median tripling
    and its tail reaching $730$\,s. Where a planner solved fewer than five
    scenarios at a level there is nothing to summarize, so those runs are drawn
    individually, one tick each.}
  \label{fig:wall}
\end{figure}
