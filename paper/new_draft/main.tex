...
\begin{figure}[t]
  ...
  \caption{Fraction of the 50 randomized scenarios each planner solved as the
    terminal uncertainty bound tightens. Marker shape and dash pattern identify
    the planner; marker fill encodes what that success cost --- the median
    primary path length at each level, normalized by the lone-agent reference
    length $L_\mathrm{ref}$, on the shared scale below the axis. One ramp
    serves every planner, so the same fill means the same detour on every
    line.}
