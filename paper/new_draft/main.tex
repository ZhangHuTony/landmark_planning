...
\begin{figure}[t]
  ...
  \caption{Fraction of the 50 randomized scenarios each planner solved as the
    terminal uncertainty bound tightens. Color encodes what that success cost:
    the median primary path length at each level, normalized by the lone-agent
    reference length $L_\mathrm{ref}$. Every planner has its own two-color
    ramp, from its base hue (paths at the reference length) to a dark shade of
    the same hue (long detours); the strip below the axis is the shared scale,
    and marker shape repeats the planner identity.}
